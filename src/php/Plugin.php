...
use ArtsEDDRD\Arts\GH\ReleaseBrowser\Browser;
...
class Plugin extends BasePlugin {
	 ...
	 * Get default strings
	 *
	 * @return array<string, mixed> Empty strings array
	 */
	 ...
	 *
	 * @return array<string, class-string> Service name => class mappings (empty - using package services)
	 */
	protected function get_core_services_classes(): array {
		return array();
	}
	 ...
	 */
	protected function register_package_services(): void {
	...
	private function get_github_token() {
		if ( defined( 'EDD_RELEASE_DEPLOY_TOKEN' ) ) {
			$token = constant( 'EDD_RELEASE_DEPLOY_TOKEN' );
			return is_string( $token ) ? $token : '';
		}

		// Use get_option directly to avoid dependency on EDD loading order
		$edd_settings = get_option( 'edd_settings', array() );
		if ( ! is_array( $edd_settings ) || ! isset( $edd_settings['edd_release_deploy_token'] ) ) {
			return '';
		}

		return is_string( $edd_settings['edd_release_deploy_token'] )
	 ...
	 *
	 * @return array<string, class-string> Manager name => class mappings
	 */
	 ...
	 */
	protected function init_package_browser(): void {
