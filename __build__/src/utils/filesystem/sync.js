...
export async function syncFiles(config, isDev = null) {
  ...
  try {
    ...
    // Sync vendor files for production builds only if WordPress plugin is enabled
    if (!isDev && shouldCreateDistFolder(config) && config.wordpressPlugin?.enabled) {
      logger.info(
    ...
    // Otherwise, sync based on configuration (only if plugin enabled)
    else if (
      config.wordpressPlugin?.enabled &&
    ...
    // Sync composer files if needed (only if plugin enabled)
    if (config.wordpressPlugin?.enabled && (shouldCreateDistFolder(config) || (isDev && config.wordpressPlugin?.target))) {
